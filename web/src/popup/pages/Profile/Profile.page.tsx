...
function ProfileData({ profile: AllergenProfile }) {
  return <Box></Box>
}
      ...
      <Box id={styles.profileHeader} h={'230px'} w={'100%'}>
        <Stack pos={'relative'} align="center" py={'xl'} px={'sm'}>
          ...
          <Group pos="absolute" right={'30px'} flex={1}>
            <ActionIcon size={'lg'} pl={'auto'} c={'black'}>
              <svg
          ...
          <Paper bg={'gray.3'} w={'80px'} h={'80px'} radius={'50px'}>
        ...
        <Stack aria-label="my-allergen-list" mt={'20px'} gap={'0'} px={'lg'}>
          {allergies.map((allergy, idx) => {
            return <AllergyItem allergy={allergy} key={idx} />
          })}
          <Button ml={'sm'} mb={'lg'} color={'olivine.6'}>
            + Add Allergy
          </Button>
          ...
          <Text fw={'light'} fs={'italic'} size="0.8rem" px={'2px'} py={'xs'}>
